...
<theme:zone name="title">
	<g:set var="subtitle"><i class="fa fa-cloud-download"></i> ${sheetInstance.name}</g:set>
...
<span class="clearfix"></span>
	...
	<ui:displayMessage/>
		...
		<blockquote>
			<h3>导出表单 <span class="muted">${sheetInstance.name}</span></h3>
			<p class="text-warning"><strong>注意</strong> 目前仅支持导出表单中的全部数据，如需合并汇总，请在导出之后在 Excel 中处理。</p>
		</blockquote>
		
		<ui:form>
			<ui:input type="hidden" name="id" value="${sheetInstance?.id}"/>
			...
			<ui:actions>
				<g:actionSubmit class="btn btn-large btn-primary" action="exportExcel" value="导出表单"/>
			</ui:actions>
		</ui:form>
